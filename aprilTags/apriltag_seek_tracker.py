"""Center a stationary AprilTag in a phone-streamed camera view using a PD
controller on a LEGO Double Motor.

This is the mirror image of apriltag_pd_tracker.py: there, a stationary
webcam watched a tag mounted on the moving car. Here, the camera (a phone
streaming video) is mounted on the car instead, and the tag sits still on
the desk. The car still only drives straight along one line -- no steering
-- so the underlying geometry (something translating along a line parallel
to something else) and the entire PD/floor/slew-limiter/distance-factor
control loop are unchanged from apriltag_pd_tracker.py. The only thing that
actually differs is the video source (see VIDEO_SOURCE below) and the
hardware-empirical constants, which will likely need re-tuning for the new
physical rig: DIRECTION_SIGN and RIGHT_MOTOR_SIGN were calibrated for a
stationary camera watching a moving tag, and there's no guarantee they're
...
DIRECTION_SIGN = 1      # flip to -1 if the car drives the wrong way -- retest
                         # this empirically, it was tuned for the other rig
...
def main():
    ...
    if not cap.isOpened():
        raise RuntimeError(
            ...
            "stream URL, confirm the phone and PC are on the same Wi-Fi network "
            "and the streaming app is running."
        )
    ...
    window = "AprilTag Seek (q to quit)"
